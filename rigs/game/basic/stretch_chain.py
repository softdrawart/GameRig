...
class Rig(BaseRig, BoneUtilityMixin):
    ...
    class MchBones(BaseRig.MchBones):
        ...
        stretch: str
        pivot: str                     # Middle mch for one bone chain
    ...
    ####################################################
    # MCH bone for one bone mechanism
    # keeps deformation bone in the middle when stretched

    @stage.generate_bones
    def make_mch_control_bones(self):
        if len(self.bones.org) == 1:
            org = self.bones.org[0]
            self.bones.mch.pivot = pivot = self.copy_bone(org, make_derived_name(org, 'mch', suffix="_pivot"), scale=0.5)
            ed_org = self.get_bone(org)
            mid_pos = (ed_org.head + ed_org.tail)/2
            put_bone(self.obj, pivot, mid_pos)

    @stage.parent_bones
    def parent_mch_control_bones(self):
        if len(self.bones.org) == 1:
            pivot = self.bones.mch.pivot
            tweak = self.bones.ctrl.tweak[0]
            self.set_bone_parent(pivot, tweak)
    ...
    def rig_mch_control_bones(self):
        if len(self.bones.org) == 1:
            ...
            self.make_constraint(pivot, 'COPY_TRANSFORMS', tweaks[1])
            self.make_constraint(pivot, 'COPY_TRANSFORMS', tweaks[0], influence=0.5)
            self.make_constraint(pivot, 'DAMPED_TRACK', tweaks[1])
            self.make_constraint(pivot, 'COPY_SCALE', 'root')
    ...
    def parent_org_bones(self):
        ...
        if len(bones.org) > 1:
            ...
            self.set_bone_parent(bones.org[0], bones.mch.pivot)
