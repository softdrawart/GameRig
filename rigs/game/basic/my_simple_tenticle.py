...
class Rig(TweakChainRig, RelinkConstraintsMixin):
    ...
    separate_rotation_axes: tuple[bool, bool, bool]
    prop_bone: str
    min_chain_length = 1
    create_tweaks: bool
    create_ctrl: bool
...
#    def rig_mch_follow_bones(self):
    ...
    def add_parameters(cls, params):
        ...
        params.separate_rotation_axes = bpy.props.BoolVectorProperty(
            size=3,
            description="Separate axes",
    ...
    def parameters_ui(cls, layout, params):
        ...
        if params.separate_rotation:
            row = layout.row(align=True)
            for i, axis in enumerate(['x', 'y', 'z']):
                row.prop(params, 'separate_rotation_axes', index=i, toggle=True, text=axis)
